...
2. **Real bug candidate (W-B):** in episode 7 the graphSnapshot-filtered projection dropped `v2ref_13051` (Dec 25) while its pair `v2ref_13053` (Dec 26) passed — target-required cells must survive graph filtering. A range-select flow cannot start with half its range.
