...
HORIZON appears when a focused requirement's target (for example a month in a date picker) lies outside the widget's currently visible window. Advance the widget with its listed navigation controls — prefer the one marked [recommended], which deterministically moves the window toward the target — until the target value becomes visible, then act on it. Never move the window away from the target. Do not re-open, re-click, or abandon the widget while the target is still outside the window.
