...
const RECOVERY_STATE_GUIDANCE: ReadonlyArray<readonly [state: string, text: string]> = [
  ...
  ['repeated_timeout_target', 'If recovery.state is repeated_timeout_target, actions are timing out: wait once, re-observe, then choose a different target; never re-click the ref that timed out, and report the unresponsiveness honestly if it persists.'],
  ['unresponsive_surface', 'If recovery.state is unresponsive_surface, actions are timing out: wait once, re-observe, then choose a different target; never re-click the ref that timed out, and report the unresponsiveness honestly if it persists.'],
  ['repeated_type_same_value', 'If recovery.state is repeated_type_same_value, the same text was already typed into that control and it is not committed: prefer one {"tool":"pick_option","ref":"<control>","text":"<requested value>"} call (the primitive opens, types, clicks the matching option, and verifies the commit), or click the matching suggestion option / press Enter instead of typing the same value again.'],
  ...
  ['calendar_click_stalled', 'If recovery.state is calendar_click_stalled, your clicks on calendar date cells are succeeding but the picker is not changing: do not re-click sibling cells blindly. Read the cell or the date field once (get) to see the picker state, use keyboard navigation (ArrowDown/ArrowUp + Enter) instead, and re-observe after each attempt.'],
  ['navigate_loop', 'If recovery.state is navigate_loop, your navigate attempts are being refused because the target equals the page you are already on — navigating again would reset entered values. Press Enter on the last typed field, click the visible submit or search control, act on the visible controls, or report honestly if the collected evidence is sufficient.'],
  ['max_step_risk', 'If recovery.state is max_step_risk, only 1-2 planner steps remain and coverage is unfinished after a rejected episode: finalize with the strongest collected evidence or escalate honestly; do not open new pages or repeat failed actions.'],
];
...
If recovery.state is repeated_timeout_target, actions are timing out: wait once, re-observe, then choose a different target; never re-click the ref that timed out, and report the unresponsiveness honestly if it persists.
If recovery.state is unresponsive_surface, actions are timing out: wait once, re-observe, then choose a different target; never re-click the ref that timed out, and report the unresponsiveness honestly if it persists.
If recovery.state is repeated_type_same_value, the same text was already typed into that control and it is not committed: prefer one {"tool":"pick_option","ref":"<control>","text":"<requested value>"} call (the primitive opens, types, clicks the matching option, and verifies the commit), or click the matching suggestion option / press Enter instead of typing the same value again.
...
If lastResult from get, inspect_region, search_page, click, type, press, navigate has lastResult.valuePreview containing the requested answer or confirming the requested state/action, return done with that value. Do not repeat the same read or mutation after successful value evidence. If the last result shows effect=none (or "no observable effect"), the runtime measured no page change from that successful action: never repeat the same tool on the same ref; choose a different mechanism (a different affordance, one bounded wait, or a read) and re-plan. The link_no_navigation signal means a link with its own target URL was clicked and the page did not navigate to it — do not repeat that click; read the link target (get) or use another affordance.
