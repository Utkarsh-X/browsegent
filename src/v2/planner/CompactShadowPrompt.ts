...
Only use indexes whose tools include the requested tool. Do not click, type, or select read-only evidence. If the needed action target is not present, use wait, scroll, search_page, or escalate dead_end with a short reason. If no index has type, never emit type; click a compatible launcher and reobserve before typing, otherwise use a non-mutating alternative.
