...
function renderMarkdownSummary(report: BenchmarkReport): string {
  return [
    ...
    `Max provider payload: ${report.summary.diagnostics?.maxProviderPayloadBytes ?? 0} bytes`,
    `Max provider user payload: ${report.summary.diagnostics?.maxProviderUserBytes ?? 0} bytes`,
    `Total provider payload/user bytes: ${report.summary.diagnostics?.totalProviderPayloadBytes ?? 0}/${report.summary.diagnostics?.totalProviderUserBytes ?? 0}`,
    `Total provider attempts/planner calls: ${report.summary.diagnostics?.totalProviderAttempts ?? 0}/${report.summary.diagnostics?.totalPlannerCalls ?? 0}`,
    ...
    `Latency ledger runs: ${report.summary.diagnostics?.latency?.runCount ?? 0}`,
    `Latency ledger p50/p95: ${report.summary.diagnostics?.latency?.p50Ms ?? 0}/${report.summary.diagnostics?.latency?.p95Ms ?? 0}ms`,
    `Latency phase totals: ${formatCounterMap(report.summary.diagnostics?.latency?.phaseTotals)}`,
